...
public class CategoriaSeeder implements CommandLineRunner {
    ...
    public void run(String... args) {
        Map<String, TipoComida> existentes = repo.findAll().stream()
                .collect(Collectors.toMap(TipoComida::getNombre, t -> t));

        for (String[] datos : CATALOGO) {
            TipoComida existing = existentes.get(datos[0]);
            if (existing != null) {
                ...
                // No es necesario llamar a repo.save() porque estamos dentro de una transacción
                // y los objetos están "managed" (gestionados) por Hibernate.
            } else {
